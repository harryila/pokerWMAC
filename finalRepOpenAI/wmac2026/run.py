...
def main():
    ...
    parser.add_argument("--ban-phrases", nargs='*', default=[], help="List of banned phrases for emergent_only robustness tests (LLMs will be instructed to avoid these in prompts)")
    parser.add_argument("--phase", type=int, default=1, choices=[1, 2, 3], help="Research phase (1=baseline, 2=lexical, 3=adversarial)")
    parser.add_argument("--constraint-level", type=str, choices=["moderate", "heavy"], help="Constraint level for Phase 2 (moderate or heavy)")
    ...
    # Determine output directory based on phase, constraint level, and hand count
    if args.phase == 1:
        # Phase 1: Baseline
        phase_name = "phase_one"
        hand_count_folder = f"{args.num_hands}_hands"
        data_dir = f"data/{phase_name}/{hand_count_folder}"
    elif args.phase == 2:
        # Phase 2: Lexical constraints
        if not args.constraint_level:
            raise ValueError("Phase 2 requires --constraint-level (moderate or heavy)")
        phase_name = "phase_two"
        constraint_folder = args.constraint_level
        hand_count_folder = f"{args.num_hands}_hands"
        data_dir = f"data/{phase_name}/{constraint_folder}/{hand_count_folder}"
    elif args.phase == 3:
        # Phase 3: Adversarial (future)
        phase_name = "phase_three"
        hand_count_folder = f"{args.num_hands}_hands"
        data_dir = f"data/{phase_name}/{hand_count_folder}"
        # TODO: Add adversarial condition subfolders when implementing Phase 3
    else:
        # Default to Phase 1
        phase_name = "phase_one"
        hand_count_folder = f"{args.num_hands}_hands"
    ...
    print(f"[WMAC] Phase: {args.phase}, Data will be saved to: {data_dir}")

    # Build game using existing infra with custom data directory
    ...
    comm_logger = CommunicationLogger(base_dir=data_dir)
    ...
    game = MixedPlayerCommunicationGame(
        ...
        logger=comm_logger,  # Use the communication logger with custom data_dir
    )
